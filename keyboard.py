...
for i in range(len(keys)):
    for j, key in enumerate(keys[i]):
        buttonList.append(Button([90 * j + 10, 90 * i + 10], key))
for i in range(len(keys1)):
    for j, key in enumerate(keys1[i]):
        buttonList1.append(Button([90 * j + 10, 90 * i + 10], key))
...
while True:
    ...
    frame = cv2.resize(frame, (1300, 880))
                    ...
                    for button in list:
                        ...
                        if (xb < x < xb + wb) and (yb < y < yb + hb):
                            ...
                            if dis < 50 and delay == 0:
                                k = button.text
                                cv2.rectangle(frame, (xb - 5, yb - 5), (xb + wb + 5, yb + hb + 5), (255, 255, 255),
                                              cv2.FILLED)
    ...
    cv2.rectangle(frame, (30, 500), (850, 550), (255, 255, 255), cv2.FILLED)
    cv2.putText(frame, text, (50, 550), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 3)
